...
const sendemail= (payload)=>
{
    ...
    //     host: '10.20.125.129', // outlook Host
    //     port: 587, // Port
    //     rejectUnauthorized: false,
    //     // secureConnection: false, // this is true as port is 465 
    //     // tls: {
    //     //       ciphers:'SSLv3'
    //     //      },      
    //     // auth: {
    //     //     user: 'notify@hellospoke.com', //outlook username
    //     //     pass: 'K6[%z#tNEC%he!h~@' // outlook password
    //     // }
    // });
...
const getKazooRequest = (req, apiKey = null) => {
    if (req['decoded'] && req['decoded'].kazoo_api_key) {
        // console.log("key here", req['decoded'].kazoo_api_key);
    }
...
var companyIdSelector:any ={
    "selector": {
        "$or": [
            ...
            {
                "pvt_type": "property",
                "propertyname": "Company A (Notify Only)",
                "enabled": true
            }
        ]
    },
...
const sendCallActivityScheduleReport=(reportdata, emails)=>
{
    ...
    reportdata.forEach(cdr => {
       ...
       //console.log(JSON.stringify(cdr));
        const cdrString = [];
        cdrString.push( 
            cdr.propertyname ,
            cdr.companyname,
        
            cdr.duringbussinesshours,
            cdr.callflowoption,
            cdr.when,
            cdr.callername,
            cdr.callernumber,
     ...
     //console.log(scheduleemailreport.filename);  
     const emailMessage = {
        from: "manishbill@gmail.com",
        to:  emaillist, 
...
const calculate_company_callsummery= async (companyid)=>
{
    ...
    var propertyData=d3c.nest()
        .key(function(d) { return d.propertyname; }) 
    ...
    var callsummery=[];
     companypropertylist.forEach(p => 
        {
            const propertydata = propertyData.find(pd=>pd.key===p.propertyname);
            const emergency_propertydata = emergency_propertyData.find(pd=>pd.key===p.propertyname );
            const emergency_property_response_data_count= emergency_property_Response_Data_Count.find(pd=>pd.key===p.propertyname );
            var rowdata:any={
                propertyname:p.propertyname,
            }
            const calloptionRows= calloptionData.find(cl=> cl.key===p.propertyname);
...
const getproperties = async (companyid)=>
{
 ...
 const updateNOtifySchedule= async(propertyid, didnumber)=>
 {
    ...
    var userIds= schedule ?await findDayScheduleUsers(accountDb,schedule,false,company,property):[];
     if (userIds.length==0) 
     {
        userIds=await findAnyNotifyDayScheduleUsers(accountDb,schedule,false,company,property);
     }   
...
const sendOptinMessage= async (tonumber, fromnumber,messagetext)=>
{
     ...
     const createindexes=async (_accountdbname)=>
     {
         try{
            ...
            var index_creation_result18= await createindex(_accountdbname,["pvt_type","elasticid"],"pvt_type_elasticid");
...
const insertescalationuserlist= (schedule,accountdbname)=>
{
...
const isduringBussinessHour=async(property)=>
{
    ...
    var to_hh= daybusinesshours.to.hh;
    if (to_hh!=12 && daybusinesshours.to.a==="pm" )
    {
        to_hh+=12;
    }
    else  if (to_hh===12 && daybusinesshours.to.a==="am" )
    {
        to_hh=0;
    }
    const from_mm= daybusinesshours.from.mm;
    const to_mm= daybusinesshours.to.mm;
    const from_a= daybusinesshours.from.a;
    const to_a= daybusinesshours.to.a;
    ...
    else
    {
        ...
        var moment_from_time= moment().tz(timezone);;
         moment_from_time.set({
            hour:  from_hh,
            minute:from_mm
        })
        var moment_to_time= moment().tz(timezone);; 
...
const parsecallinfoinitdata = async(accountDb,callinfolog)=>
{
    var promise1 = new Promise(async (resolve , reject )=>{
        ...
        console.log("inserting initial report");
        await insertreportdata(reportdata,accountDb);
       // 
        resolve (  reportdata);
    })
    ...
    var promise1= new Promise(async (resolve , reject )=>
    {
        switch (callinfotype)
        {
            case "callinit":
                {
                    if (callinfo.when.toLowerCase()==="live")
                        var result =await parsecallinfoinitdata(companydb,callinfo);
                        console.log("parsing end",callinfotype ) 
                        resolve(result); 
            ...
            case  "agentrespnse":   
            {
                resolve(await parseagentrespnse(companydb,callinfo));
                console.log("parsing end",callinfotype ) 
                break;
            }
            case  "pin":   
            {
                resolve( await parseagentpin(companydb,callinfo));
                console.log("parsing end",callinfotype ) 
                break;
            }
            ...
            case "callrecording":
            {
                resolve( await parsecallrecording(companydb,callinfo));
                break;
            }

            case "callend":
            {
                resolve(await parsecallend(companydb,callinfo));
                break;
            }
            case "messagerecordingend":
            {
                resolve( await parsemessagerecordingend(companydb,callinfo));
                break;
            }
...
const parselivecallinfologdataforreport=async(companydb,guid)=>
{
    ...
    var promise1= new  Promise(async (resolve, reject) => {
        ...
        for (var i=0 ; i<callinfologs.length;i++)
        {
            ...
            console.log("parsing1111111111 end",callinfotype )
        }
    ...
    if (result_callinfo)
    {
        await insertreportdatatoelastic(companydb,result_callinfo);
        const  notifycalllog= await getNotifyCallLog(companydb,result_callinfo);
        ...
        if (callinfotype==="callinit")
        {
            ...
            if (err) {
                ...
                if (when === "live" && callinfotype==="callend" && reason==="NORMAL_CLEARING")
                {
                    ...
                    {
                         removeInitialCallRecord(callinfo,companydb );
                         resolve (await parselivecallinfologdataforreport(companydb,callinfo.guid));
                    }
                }
                else if (when === "notify" &&(callinfotype==="ruleexecution" || callinfotype==="ring") )
                {
                   ...
                   resolve (insertnotifycallinfologtoreport(callinfotype, callinfo, companydb));
                }
                else if (when=== "agentcall" && callinfotype ==="agentaction")
                {
                    resolve ( parseagenaction(companydb,callinfo));
                }
               else if (when=== "agentcall" && callinfotype ==="callbackend")
                {
                    resolve (parseagentcallend(companydb,callinfo));
                }
                else if (when=== "agentcall" && callinfotype ==="pin")
                {
                    resolve (parseagentpinfornotify(companydb,callinfo));
                }
...
const resetpasswordemail = (payload) => {
    // let transporter = nodemailer.createTransport({
    //     host: 'smtp-mail.outlook.com', // outlook Host
    //     port: 587, // Port
    //     secureConnection: false, // this is true as port is 465
    //     tls: {
    //           ciphers:'SSLv3'
        ...
        <div
            ...
            <div class="row" style="margin-left:0px;margin-right:0px;">
                <div style="font-size: 14px; color:#003A5d;  margin-top: 30px;">
                    Hi , <br /> We received a request to reset your HelloSpoke password.
                </div>
...
const vmboxemail = (payload,reportdata) => {
  ...
  const filename = reportdata.messageid ? reportdata.messageid : '';
  
  const filetype = 'wav';
  const filesize = "";
  const boxid= reportdata.boxid;
  const audioserverurl= `http://voip-auto-testing.hsnotify.com:3000/v1/${boxid}/${filename}`
  const fileurl =reportdata.messageurl ? reportdata.messageurl : audioserverurl;
...
const propertyname= reportdata.propertyname;
  ...
  const voicemailboxname= `${callflowoption} Voicemail `;
    ...
    payload.data.emaillist.forEach(email => {
        let mailOptions = {
            from: 'Hellospoke Notify <notify@hellospoke.com>',
            to: email,
...
app.get('/forgotpassword/company/:name/user/:email', async (req, res) => {
  //  console.log("here is the user object 11111111111");
    ...
    companyIdSelector.selector.$or[1].propertyname= accountname;
...
// create users delete
app.delete('/companies/:companyid/property/:propertyid/users/:userid/:removeall', async(req, res) => {
...
app.post('/companies/:companyid/properties/:propertyid/users', validateJWT, async (req, res) => {
    ...
    const kazooacccountname= primarykazooaccount ? primarykazooaccount.name:payload.data.accountname;

    if (!primarykazooaccount) //if not recieved primarykazooaccount in request
    {
        primarykazooaccount={
            id:kazooacccountid,
...
app.get('/company/:comapnyid/user/:userid/scheduleactivityreport', async (req, res) => {
    console.log("new");
    const companyid=req.params.comapnyid;
    const accountDbName= parseAccountToDatabaseName(companyid);
    const accountDb = nano.use(accountDbName);
...
app.get('/company/:comapnyid/user/:userid/scheduleemailreport', async (req, res) => {
    console.log("afas");
    const companyid=req.params.comapnyid;
    const accountDbName= parseAccountToDatabaseName(companyid);
    const accountDb = nano.use(accountDbName);
...
app.get('/companies/:id/properties/:propertyid/users/notadded',(req, res) => {
    const companyid= req.params.id;
   const  propertyid= req.params.propertyid;
    const accountDb = nano.use(parseAccountToDatabaseName(companyid));
  console.log("\n accountDb: ",parseAccountToDatabaseName(companyid));
...
app.get('/companies/:id/masterusers/notadded',(req, res) => {
    const companyid= req.params.id;
...
app.get('/companies/:companyid/callsummery', async(req, res) => {  
...
app.get('/companies/:companyid/properties_avg',async (req, res) => {
...
app.post('/companies/:companyid/properties/:propertyid/addusers', async(req, res) => {
...
app.post('/companies/:companyid/addmasterusers', async(req, res) => {
                    ...
                    new Promise(async(resolve, reject) =>
				{
                   ...
                   var usernamesuffix= suffix==1? callflowdeviceusernamesuffix :propertydeviceusernamesuffix;
                    ...
                    else
                    {
                        ...
                        putDataDevice.data.caller_id.external.name= payload.propertyname;
                        if (suffix==="1")
...
app.post('/companies/:companyid/properties/:propertyid/escalationuserlist', validateJWT, (req, res) => {
    console.log("\n\nescalationuserlist\n");
    //const companyid= req.params.companyid;
    const propertyid= req.params.propertyid;
    //const scheduleid=req.params.id;
    const payload =JSON.parse(req.body.payload)  ;
    const dbname=parseAccountToDatabaseName(propertyid);
    const result =insertescalationuserlist(payload,dbname)   ;
    res.send(result);
...
const getSchedule_for_callflowsoptiontype= async(accountDb,callflowsoptiontype,scheduletype)=>
{
    ...
    console.log(JSON.stringify( schdulelist))
    return schdulelist;
...
const findSchedules = async (accountDb:any,isliveschedule:boolean,
    ...
    const schedulePromise = new Promise<any[]>((resolve, reject) => {
        // console.log(currentUserSelector);
    ...
    // console.log(`Starting to search for users in set`, userIds);
    const scheduledocs = await schedulePromise;
    var document= scheduledocs.length>0 ?scheduledocs[scheduledocs.length-1]: {};
...
const findNotifyEscalationSettings = async (property:any): Promise<any> => {
    ...
    // console.log(`Starting to search for users in set`, userIds);
        ...
        escalationSettingsdocs.forEach(setting => {
            setting.escalationsettings.forEach(escalationsetting => {
         //   console.log(escalationsetting.time);
           // console.log(escalationsetting.number);
            var temp= {
                "name": `${setting.first_name} ${setting.last_name}`,
                "waittime": escalationsetting.time*40,
...
const updateemrtdata=async()=>
{
    ...
    var dbnames:any=await getaccountdbnames();
    dbnames= dbnames.filter (d=>  account_db_pattern.test(d));
    // && d===parseAccountToDatabaseName("0101240b6759822c194d4e5c95d32117"));
    for (var i=0; i<dbnames.length;i++)
    {
        const dbname=dbnames[i]
        console.log(dbname); 
...
{
    var today =moment().tz(property.timezone).startOf('day');
   const starttime_utc_unix= today.clone().utc().unix();
    const endtime_utc_unix= today.clone().add(1,"days").add(455,"minutes").unix();
    const company=await getcompanyInfo(property.companyid); 
    
   var dayschedules = await findUsersForhandOffRules(property,starttime_utc_unix,endtime_utc_unix);
   ...
   for (var i=0; i<dayschedules.length;i++)
   {
      ...
      if(property_day_schedule)
      {
            ...
            var schedule_start_time= today.clone().add(from_hh, "hours").add(from_mm, "minutes");
            ...
            const schedule_end_time= today.clone().add(to_hh, "hours").add(to_mm, "minutes");
...
const findAnyNotifyDayScheduleUsers = async (accountDb:any,schedule:any,islivereply,company,property): Promise<string[]> => {
...
app.get('/property/:propertyid/:incidentid/notes',async (req, res) => {
...
app.post('/property/:propertyid/:incidentid/notes', (req, res) => {
    console.log("\n\n incidentid notes\n");
...
app.get('/company/:companyid/autosearch',async (req, res) => {
...
app.get('/property/:propertyid/autosearch',async (req, res) => {
    console.log("\n\n get reportdata  log\n");
    const propertyid= req.params.propertyid;
...
app.get('/property/:propertyid/chart/:type/:bussinesshours/:nonbussinesshours/:startime/:endtime',async (req, res) => {
...
app.get('/property/:propertyid/reportdata',async (req, res) => {
    console.log("\n\n get reportdata  log\n");
    let body = {
        size: 20,
        from: 0,
    ...
    app.post('/webhook/voicemail', async (req, res) => {
        ...
        const result =await insertdtmfinfo(payload,companydbname)   ;
...
app.post('/property/dtmfinfo', async (req, res) => {
    ...
    const result =await insertdtmfinfo(payload,dbname)   ;
