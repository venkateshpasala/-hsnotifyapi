    ...
    const resultpromise=  new Promise (async(resolve, reject)=>{
        Request(options, function (error, response, body) {
            ...
            else
            {
               ...
               // console.log(JSON.stringify(result.map(r=>r.responsetime)) +' // '+ 'succuss');
                resolve(result)
              //  varresolve(body.hits.hits)
...
app.get('/forgotpassword/company/:name/user/:email', async (req, res) => {
  //  console.log("here is the user object 11111111111");
    ...
    const companyIdSelector:any ={
        ...
        const propertyIdSelector:any ={
            "selector": {
                "pvt_type": "property",
                "kazoopropertyname": accountname,
                "enabled": true
            },
            "fields": [
                "_id",
     ...
     for (var i=0; i<dbnames.length;i++)
     {
         var dbname= dbnames[i];
            ...
            const accountDb = nano.use(dbname);
             ...
             account=await getdocumentbyproperty(accountDb,companyIdSelector); 
             if (!account) //if user created in property 
                 account=await getdocumentbyproperty(accountDb,propertyIdSelector); 
    ...
    // console.log("id " + _id);
     ...
     async (e, r, b) => {
         if (e) {
            ...
            var result= await insertUser(_userobj, accountDb);
...
const elasticSerachDelete= async()=>
{
    ...
    const p={"payload":{
        querystring: '((companyid:fe79a4ccb8fbe929070c4d70a4ffabdf) )',
        starttime: 1603305000,
        endtime: 1606156199
      }};
      console.log("result1");
   ...
   result1= result1.filter (h=>h._source.responsetime==="-" );
   console.log(result1.length)
   var i=0;
   for (i=0; i<result1.length;i++)
    {
       var r= result1[i]    
       console.log(`${i}  ${r._source.companyname} ${r._source.responsetime} `);
  ...
  //  var rep= reports.find(r1=>r1.elasticid===r._id)
   ...
   // await insertreportdata(rep,accountdb);
    var fields = {
        responsetime:0,
        responsetime_old:r._source.responsetime,
        updateddoc:true
    }
